...
static void video_display(VideoState* is)
{
    ...
    LOGE("g_nIsPlay=%d", g_nIsPlay);
    if(g_nIsPlay == 1)
    {
        SDL_RenderCopyEx(renderer, pStopTexture, NULL, &nStopRect, 0, NULL, SDL_FLIP_NONE);
    }
    else if(g_nIsPlay == 0)
    {
	...
	for (;;) {
		...
		case SDL_MOUSEBUTTONDOWN:
			    ...
			    && event.button.y >= nPlayRect.y && event.button.y <= (nPlayRect.y + nPlayRect.h))
			{
				...
				{
					g_nIsPlay = 0;
					LOGE("button(%d, %d)g_nIsPlay(1->0)",event.button.x, event.button.y);
					cur_stream->force_refresh = 1;
				}
				else if(g_nIsPlay == 0)
				{
					g_nIsPlay = 1;
					LOGE("button(%d, %d)g_nIsPlay(0->1)",event.button.x, event.button.y);
					cur_stream->force_refresh = 1;
				}
...
int main(int argc, char** argv)
{
    ...
    int n_argc = 2;
    char *p_argv[] = {"app_process","big_buck_bunny.mp4"};

	LOGE("enter main(argc=%d)",argc);
    if(argc > 0)
    {
        LOGE("enter main(argv[0]=%s)",argv[0]);
    }
	init_dynload();
    ...
    LOGE("play big_buck_bunny.mp4(%s, display_disable=%d)", input_filename, display_disable);
	...
	if (SDL_Init(flags)) {
        ...
        LOGE("(Did you set the DISPLAY variable?)\n");
